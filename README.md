...
> 具体看对应文件夹
>
> ---  openwrt	运营商光猫定制lxc容器下运行的openwrt
>
> ---  network    各地区网络认证方式和链接VLAN信息
>
> ---  iptables     iptables/ip6tables 规则维护



# 基础说明

> 目前认证方式分为 LOID 账号密码、PASSWORD 密码验证、光猫 SN 序列号、光猫 MAC 物理地址等，运营商会选取一种或者几种组合验证。光猫 SN 序列号和 MAC 地址在会标在光猫底部，LOID 账号密码需要从运营商客服 / 宽带安装师傅那获取，同时也需要宽带拨号 / 电话 / IPTV 对应的 Vlan 值。每个城市或者省份的同一运营商，默认采用同一套认证方式，各区域 Vlan 通常也是一致的，可以通过分享的方法简化操作，大家可以在`issue`留言分享，本文会实时更新数据。 
